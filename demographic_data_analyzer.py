...
def calculate_demographic_data(print_data=True):
    ...
    higher_education = df.query(
        "education in ['Bachelors', 'Masters', 'Doctorate']"
    ).shape[0]
    lower_education = df.query(
        "education not in ['Bachelors', 'Masters', 'Doctorate']"
    ).shape[0]
    ...
    rich_percentage = (
        (df.query("`hours-per-week` == 1 and salary == '>50K'").shape[0])
        / num_min_workers
        * 100
    )
